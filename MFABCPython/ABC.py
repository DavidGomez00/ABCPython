...
class ABC:
    ...
    def send_employed_bees(_self):
        ...
        while (i < _self.conf.FOOD_NUMBER) and (not (_self.stopping_condition())):
            ...
            # Se escogen 4 soluciones distintas aleatorias distintas que no sean la mejor solución
            neighbours = np.random.choice(np.delete(np.arange(_self.foods.shape[0]), [_self.globalIndex, i]), 4, replace=False)
            n1, n2, n3, n4 = neighbours[0], neighbours[1], neighbours[2], neighbours[3]
                ...
                _self.solution[_self.param2change] = (_self.foods[n1][_self.param2change]) \
                    + (_self.foods[n1][_self.param2change] - _self.foods[n2][_self.param2change]) \
                        * (random.uniform(-0.75, 0.75)) \
                            + (_self.globalOpts[_self.param2change] - _self.foods[n1][_self.param2change]) \
                                * (random.uniform(0, 0.5))
            # Se usa ABC/best
            else:
                _self.solution[_self.param2change] = (_self.globalOpts[_self.param2change]) \
                    + (_self.foods[n1][_self.param2change] - _self.foods[n2][_self.param2change]) \
                        * (random.uniform(-1, 1))
            ...
            if (_self.FitnessSol > _self.fitness[i] and _self.conf.MINIMIZE == True) or (_self.FitnessSol <= _self.fitness[i] and _self.conf.MINIMIZE == False):
                ...
                # Se actualiza el evolution ratio de las fórmulas
                '''TODO: Cosas con los degree'''
